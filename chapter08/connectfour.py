...
class C4Board(Board):
    ...
    class Column:
        ...
        def push(self, item: C4Piece) -> None:
            if self.full:
                raise OverflowError("Trying to push a piece to full column")
    ...
    def __init__(self, position: Optional[List[C4Board.Column]] = None,
        ...
        if position is None:
            self.position: List[C4Board.Column] = [C4Board.Column()
                                                   for _ in range(C4Board.NUM_COLUMNS)]
        else:
    ...
    def _count_segment(self, segment: List[Tuple[int, int]]) -> Tuple[int, int]:
        ...
        for column, row in segment:
            if self.position[column][row] == C4Piece.B:
                black_count += 1
            if self.position[column][row] == C4Piece.R:
                red_count += 1
