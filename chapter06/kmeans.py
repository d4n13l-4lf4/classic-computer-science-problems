...
from chapter06.DataPoint import DataPoint
...
class KMeans(Generic[Point]):
    ...
    def _dimensions_slice(self, dimension: int) -> List[float]:
        return [x.dimensions[dimension] for x in self._points]

    def _zscore_normalize(self) -> None:
        zscored: List[List[float]] = [[] for _ in range(len(self._points))]
        for dimension in range(self._points[0].num_dimensions):
            dimension_slice: List[float] = self._dimensions_slice(dimension)
    ...
    def _random_point(self) -> DataPoint:
        ...
        for dimension in range(self._points[0].num_dimensions):
            values: List[float] = self._dimensions_slice(dimension)
    ...
    def _generate_centroids(self) -> None:
        for cluster in self._clusters:
            ...
            for dimension in range(cluster.points[0].num_dimensions):
                dimension_slice: List[float] = [p.dimensions[dimension]
...
if __name__ == '__main__':
    ...
    for index, cluster in enumerate(test_clusters):
        print(f"Cluster {index}: {cluster.points} Center: {cluster.centroid}")